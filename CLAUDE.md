...
## ⛔⛔ Guideline 5.6 拒审：包里不许有「审核期不会执行」的远程代码分支（2026-08-07 实锤）

**Dungeon Sweep（minesweeper）1.0 被按 5.6 开发者行为准则拒**，原话是
「app 含有似乎在**审核过程中被刻意隐藏**的功能 / 与欺诈行为相关的异常模式」。
苹果的 5.6 模板从不点名具体功能 ⇒ 下面是**排查出的唯一对得上的东西**，不是苹果明说的：

`engine/portal.js`（游戏门户广告适配）会 `createElement('script')` 从
`html5.api.gamedistribution.com` 拉 SDK 执行，而它的 `active` 要求 `!Platform.isNative`
⇒ **原生下永不执行、审核员在设备上怎么点都看不到**，开关还是外部可拨的
（`?portal=gd` / `window.__PORTAL__`）。意图无辜（网页版发门户用），
但**那个意图在二进制里看不出来**，静态分析读到的就是「可被外部开关激活、
下载执行第三方任意代码、且审核期不可见」。⚠ **七个游戏的包里全带这段**，
含已上架的 snake / blockblast / solitaire ⇒ 是**账号级**风险，不是单个 app 的事。

**规矩**：原生包里不许有任何动态/远程代码执行的**痕迹** —— 苹果看的是**代码里有什么**，
不是**运行时会不会走到**。`tools/build-www.cjs` 已落两道：
