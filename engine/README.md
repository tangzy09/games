...
| input.js | `Input` | tap→hitTest→onAction；滑动/方向键→onSwipe（canSwipe 门控）；`liveSwipe:true` 时 touchmove 即触发 onSwipe（实时游戏，opt-in，回合制游戏不受影响）；**`onHold(x,y)`/`onHoldMove(x,y)`/`onHoldEnd(x,y)`**（opt-in，「按住预览、松手才落」类玩法用；触摸+鼠标两条路，指针离开画布/触摸被打断时发 `onHoldEnd(-1,-1)` 让游戏清预览而不落子）。⚠ **`onAction` 的 `dist<10 && dt<500` 窗口对这三件套不适用**——按住预览按定义 `dt` 是好几秒，那一手在 `onAction` 里会被静默丢掉；两套是并行触发的，opt-in 的游戏自己在 `onHoldEnd` 里去重 |
