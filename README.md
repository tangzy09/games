...
| `connect4` | **✅ 已上线 <https://four.ai-speeds.com>；P1~P5 全部交付 —— 能玩、有手感、一家人能一起玩下去，下完能看懂自己哪一手走坏了，还有求解器自动出题的课程**（人机/双人、按住预览、落子物理与随深度变调的音、威胁高亮与双威胁光环、赢局逐段揭示、减弱动态+舒适模式、中英双语；**让子 / 儿童档 / 对坐模式+猜先 / 双人局对方同意才悔 / 限时模式（默认关）**）。完美求解器通过 Allis 1988 外部真值门禁，20 级明面 AI 阶梯，求解器跑在 Worker 里。**P3**：分层提示 / ✨妙手 / 赛后复盘（胜负曲线+转折点+从这一步重来）/ 精准度与最高纪录，全部**永远免费零广告**；地基是「边打边算」⇒ 终局那一刻真值已算完 100%。**P4/P5**：课程系统（自动出题/判分/诊断推课）· 元游戏（等级/双口径统计/成就/星级/每日任务）· 变现闸门（前 50 盘零插屏、输局永不出）。⚠ 还差**美术** ⇒ iOS 壳与上架未起。⛔ `Connect 4` 是活商标，泛型词用 `Four in a Row` |
