...
> ## ✅ 已执行完毕（2026-08-01）—— 本文件转为**历史记录**
>
> 九个任务全部完成，**游戏可以玩了**。⚠ 权威规格是 `games/connect4/DESIGN.md`，不是本文件。
>
> **计划里被实测纠正的（别照抄）**：
> - `EngineClient.ai` 是**位置参数** `ai(moves, tier, seed)`，不是我写的对象参数
> - 「主线程不加载 bitboard/solver/ai/book」**做不到**——`state.js` 要 `ConnectAI.paramsDigest`，`ai.js` 顶层就吃 `Bitboard/RulesClassic/Solver/裸 PRNG`。改成六模块全量加载 + 「主线程一行搜索都不跑」
> - `Book.load(url)` 自己 fetch，**没有** `load(arrayBuffer)` 重载
> - ⭐ **活性探针只回 `pong` 不够**：漏 `book.js` / `ai.js` 时 Worker 起得来、`onerror` 恒 null、裸 ping 照样有回音 ⇒ **完全静默**。必须做成横跨六依赖的真调用自检
> - ⭐ **超时只该管启动**：给正常要 1.7–4 秒的搜索设 3 秒超时，会把工作正常的求解器报成 dead
> - 「n≤9 无库要几十分钟」不准——**实测 4–5 秒**，几十分钟是空盘那一档
> - ⭐ **E2E 的三条顶档断言是恒真的**：`PLAY_AI` 把 `gameNo` 归零，交替规则在第 0 局本来就给 `true` ⇒ 删掉顶档强制逻辑一条都不会红。已补奇数局号探针 + 反向对照
>
> **计划没预见、实测才发现的**：
> - ⛔ `engine/*.js` 在本游戏一直**没挂 `?v=N`**（另外四个游戏都有），而 T3 刚改过 `engine/input.js` ⇒ 老玩家命中旧缓存 = **一子都落不下去且零报错**。已补齐
> - 胜利帧的圆形光晕把六边形**完全盖住**、看起来成了对手的造型 —— **双编码在最重要的一帧上反过来骗人**，脚本全绿，看图才抓到
> - `window.GameGlobal` 恒 `undefined`（顶层 `const` 是词法绑定），E2E 里症状是 **`waitForFunction` 等到超时**而不是报错
>
> **下一步**：P2b（手感与无障碍）需要**另起一份 writing-plans**。

## 执行进度（滚动更新）

| 任务 | 状态 | 备注 |
|---|---|---|
| T1 页面骨架 | ✅ `cf3f1a0` | 中英双语已看图确认（无 key 原文泄漏）；`check-locales` 0 fail |
| T2 状态层 | ✅ `92e2cdb` | 存档 42 手 **194 字节**（⚠ 规格原写「几十字节」已订正为实测值）；`St.play` 非法列**抛错**、`deserialize` 脏输入**永不抛**（两侧纪律相反是故意的：前者调用方是我们自己的 UI，后者输入不可信） |
| T3 引擎输入 | ⬜ | ⚠ 六游戏共用，见上面「风险 1」 |
| T4 Worker | ⬜ | ⚠ 见「风险 2」 |
| T5 渲染 · T6 主循环 · T7 商标门禁 · T8 E2E · T9 收尾 | ⬜ | |

### ⚠ 已知接线欠账（T2 交接，**T6 必须补**）

`js/state.js` **还没进 `index.html` 的 `<script>` 列表，也没进 `tests/test-browser-globals.js` 的 `LOAD_ORDER`**（那两个是 T1 的文件，本仓多会话并行，T2 没动）。
⇒ **T6 接 `main.js` 时必须把 `state.js` 补进这两处**，**顺序排在 `ai.js` 之后**（它要 `ConnectAI.paramsDigest`）。
⛔ 不补 = **浏览器路径零覆盖** —— 那正是 P1 终审抓到阻塞项（`root.PRNG` 恒 undefined）的同一个盲区。T2 自己在 `test-state.js` 里做了等价的沙箱真调用兜底，但那不能替代 `LOAD_ORDER` 里的顺序验证。
