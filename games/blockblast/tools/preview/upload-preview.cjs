// games/blockblast/tools/preview/upload-preview.cjs — 把 App Preview 传进 App Store Connect
//
// 用法：node games/blockblast/tools/preview/upload-preview.cjs <appStoreVersionId> [locale...]
...
const FILE = 'C:/tmp/blockblast/preview/cubeblast-preview-en.mp4';
const PREVIEW_TYPE = 'IPHONE_67';          // 6.7" 竖版槽位（886×1920 / 1080×1920）
const POSTER = '00:00:19:00';              // 海报帧：500 张画像收集墙（最好看的一幕）
  ...
  for (const d of locs) {
    ...
    const res = await asc.api('POST', '/v1/appPreviews', {
      data: { type: 'appPreviews',
        attributes: { fileName: `cubeblast-preview-${loc}.mp4`, fileSize: buf.length, mimeType: 'video/mp4' },
