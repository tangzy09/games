...
## ⚡🎉 爽感批（2026-08-04，用户点名四件）

1. **快速放置 → 时间加分**（`core.js` 的 `FAST_MS`/`speedBonus`）：2.5s 内连续落子，链 +1，
   给 5/10/…/25 分封顶。⛔ **纯增益**：慢了不扣分、不断 streak、**UI 上绝不画倒计时条** ——
   DESIGN §0.7「永不加倒计时」没被推翻，区别是「惩罚慢」vs「奖励快」（DESIGN §0.6b 写清了边界）。
   ⛔ **每日/挑战局不给**（同种子分数必须可比，与礼包手同一个先例）。
   ⚠ 时间戳**从外部传入**，core 仍是纯函数 ⇒ **不传 = 不给** ⇒ sim/参考 AI/verify-levels
   行为一个字节没变（中位分仍 2991，基线 2971）。撤销要回滚，否则「撤销→重放」可无限刷分。
2. **消行特效按条数递增**：粒子 5/7/9/11/13 · 屏震 8→24（原来封顶 14，四条和两条几乎没差别）·
   **冲击波环 0/1/2/3/4 个**（⭐ 这才是一眼看得出档位的那一维，粒子多寡人眼其实不敏感）·
   整行/列先亮一道**光带** · 4 条起环与光带转金色 · 赞美词逐档放大。
3. **清屏特效分三档**：sweep 2 环 · deep 3 环 + 星雨 · **perfect 4 环 + 星雨 + 全屏闪**
   （⛔ 全屏闪只留给 perfect，到处用就不值钱且晃眼）。
4. **过关画面可爱化**：天使来祝贺（按关号确定性挑一张已解锁的 + 圣光）· 三颗星**逐颗弹出**
   （冲过头再回弹，线性放大看着像卡顿）· 赢的瞬间撒**彩色纸屑**。

新验收 `node games/blockblast/tools/shot-clearfx.cjs`：摆盘 → **真落子** → 特效播到一半时截图，
五档各一张 + 断言消行数递增。⛔ 两个坑都是它自己踩出来的：
**① 必须走 `onPlace` 不能直接调 `Core.place`** —— FX 在 main 的 `consume(events)` 里触发，
绕过去就只改状态、一个特效都不播（消行数对、`FX.busy()` 恒 false，和「E2E 用 dispatch 绕过
真实点击 = 假绿」是同一个坑）；**② 测试盘面底部要留几行填满的格子**，否则消完棋盘几乎全空 ⇒
每档都顺带触发 DEEP SWEEP，清屏特效盖在消行特效上，梯度根本看不出来。
⚠ 实拍还抓出一个既有 bug：toast 防重叠阈值 30px **盖不住 40px 的大字** ⇒
「DEEP SWEEP!」和「Unbelievable!」几乎完全重叠，已提到 46。
