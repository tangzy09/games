...
const files = {
  ...
  // ⭐ 妙手（P3 T4 · DESIGN §3.4「只有 1 列不输、而你找到了」）：
  //     A5 → E6 → A6 的大调上行，收尾**单音长鸣**。
  //     ⭐⭐ 与 fork 的区分是**语义**的，不只是音色：
  //       双威胁 = **两条路** ⇒ fork 收尾故意是两个音同时响（G5+D6 纯五度）；
  //       妙手   = **唯一解** ⇒ 这里收尾只有**一个**音。声音本身就把两件事说清楚了。
  //     ⚠ 频率也刻意错开（A 系列 vs fork 的 G/D 系列）⇒ 连着响也分得出来。
  //     ⛔ 别再让妙手借用 fork 音：两件不同的事听起来一样，玩家分不清，
  //       而且会污染 e2e-p2b-t5 那条「fork 音恰好响一次」的门禁（2026-08-06 实锤）。
  brilliant: norm(concat(note(880, 0.05, 0.34, 0.004, 0.02),
    note(1319, 0.05, 0.34, 0.004, 0.02),
    note(1760, 0.24, 0.40, 0.005, 0.12)), 0.82),
