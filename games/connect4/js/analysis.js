...
(function (root) {
  ...
  function fireIdle() {
    ...
    // ⛔⛔ **必须重新取一次忙闲**，⛔ 绝不能把回调**之前**那张快照（busy）写回 wasBusy：
    //   监听者完全可能在回调里**又排了活** —— 教程「这道题不合本课概念 ⇒ 换一道」走的正是这条。
    //   写回旧快照的话，刚排进去的那条会被记成「不忙」⇒ 它算完时 wasBusy 已经是 false
    //   ⇒ `wasBusy && !busy` 永假 ⇒ **onIdle 再也不响一次**。
    //   后果（2026-08-07 试玩当场复现）：教程 16 课里 12 课**永远停在「正在出题…」**，
    //   盘面一个热区都没有 —— 玩家看到的就是「点了没反应」。而 progress() 是 done=total
    //   （活其实全干完了），⛔ 零报错、截图看起来也只是「在加载」。
    wasBusy = inflight || queue.length > 0;
  }
