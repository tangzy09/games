...
//        ［复盘］是**真按钮**（P3-T5 已填内容；P2b 时它是不注册热区的留位 ——
//        红线始终是「这颗按钮不许是假的」，只是兑现方式随实现变了）·
...
(async () => {
  ...
  // ⭐⭐ **判据在 P3-T5 反过来了（2026-08-06），而红线一个字没松。**
  //   P2b 时这里断言的是「⛔ 一个热区都没注册」——理由是**假按钮（点了没反应）比没按钮更伤**，
  //   而当时复盘还没实现。P3-T5 把内容填上了 ⇒ 它现在**必须**是真按钮。
  //   ⇒ 红线始终是同一条：**这颗按钮不许是假的**。P2b 靠「没热区」兑现，P3 靠「点得进复盘页」兑现。
  //   ⚠ 输局那颗写着［从那一步重来］，但 action 同为 REVIEW（进同一个复盘页，
  //     ［从这一步重来］在页内）—— ⛔ 别做成两条不同的路。
  ok(await hasHit('REVIEW'),
    '⭐ ［复盘］现在是**真按钮**（P3-T5 已填内容）⇒ 必须注册热区'
    + ' —— ⛔ 红线不变：这颗按钮永远不许是「点了没反应」的假按钮');
  ok(!(await hasHit('REPLAY_FROM')),
    '⭐ ［从这一步重来］只在**复盘页内**出现（那里才知道转折点在哪）'
    + ' —— 结算屏这一层不该有它');
