...
2. **弹药与刷行都从「盘面现有值」抽样**（`pickFromBoard`）——不是公式算区间。偏小抽样（`AMMO_BIAS=2` / `SPAWN_BIAS=3`，`r^bias` 权重）：主要靠「射小鱼 → 级联长大」推进（经典 2048 射手的正路），大鱼也抽得到但概率低。空盘兜底 `TILE_MIN`。
...
**蒙特卡洛口径**：无尽刷分**没有「可赢性」可验**（区别于 mines 的必胜门禁）——`test-sim.js` 只验「不变量成立 + 无退化的秒死/永生局 + 局长分布合理 + resolve 收敛」。当前随机瞎打 300 局（指数合并 + 盘面抽样 + 击中优先 三条新规则后）：**局长 min=24 / 中位=48 / 均=51 / max=102**；**最高鱼中位=512（管眼鱼）、max=16384（虎鲸）**。连通块大小分布：2 连 76% / 3 连 14% / 4 连 4% / 5+ 连 3%（指数大奖够得着但不廉价）。
