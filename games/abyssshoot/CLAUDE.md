...
1. **连通块合并 = 指数奖励**（`findComponents` + `resolve`）：四邻相连的同值块 N 个（N≥2）→ **合并 N-1 次 → `V × 2^(N-1)`**，落在**锚点 = index 最大（最低）、再 c 最小（最左）**。3 连=4 倍、4 连=8 倍、5 连=16 倍——**「憋大团」是核心爽点与赌注**。结算是**确定性单趟**：找出当前所有块 → 同趟全部结算 → 一次 `gravityUp` → 重扫，直到无块。**连锁倍率 = 迭代轮数**，计分 `gained += 新值 × 当前轮数`。
   - ⚠ **2026-07-11 玩家实战推翻的旧规则**：原本不论 N 一律塌成 `×2` → **凑的团越大亏得越多**（5 个 4 → 8，总值 20→8，亏 12）。反玩家、一上手就撞到。**已废弃，勿回退。**
2. **基准档 `baseTier`：弹药与刷行「都」挂它**（`genAmmo` + `spawnTile`）：`base = 2^(log2(maxTile) − BASE_SPAN)`（`BASE_SPAN=5`，平衡关键钮）。弹药 ∈ `[base, base×4]`，刷行的鱼 ∈ `[base, base×2]`，随进度**一起上浮**。
   - ⚠ **防「死墙」的硬不变量，也是 2026-07-11 玩家实战撞出的致命 bug**：旧实现弹药挂「盘上最小值」、而刷行**恒生 2/4** → 刷行不停把最小值**钉死在 2** → 弹药永远 `{2,4,8}` → 列底压着的 256 **永远合不动**，成为不可消的死墙，盘面只涨不消、被活活顶死。**基准必须挂单调上浮的进度信号（`maxTile`），且刷行与弹药同挂一个基准。** 单测有「刷行的鱼必须落在弹药可达区间内」的断言守着。
...
4. **唯一失败条件**：任意列 `length > rows` 即死（学 mines 的「一条干净规则」）。没有命数、没有别的死法。
...
`core.js` 顶部：`PREVIEW=3`（预览发数）、`AMMO_WINDOW=3`（弹药档窗）、`SPAWN_EVERY=6`（每 N 发刷一行）、`BASE_SPAN=5`（基准档回退档数，**最关键的平衡钮**）、`TILE_MIN=2`、`cols=5`/`rows=9`。鱼梯 17 档（`tiles.js`，顶到 131072 皇带鱼）；`render.js` 的 `PAL.tiers` 必须 ≥ 鱼梯档数，否则高档取模绕回浅色撞色。

**蒙特卡洛口径**：无尽刷分**没有「可赢性」可验**（区别于 mines 的必胜门禁）——`test-sim.js` 只验「不变量成立 + 无退化的秒死/永生局 + 局长分布合理 + resolve 收敛」。当前随机瞎打 300 局（指数合并 + baseTier + 击中优先 三条新规则后）：**局长 min=21 / 中位=59 / 均=61 / max=138**；**最高鱼中位=256（鮟鱇）、max=65536（抹香鲸）**——瞎打都能摸到深海巨兽，说明死墙已堵、连锁能滚雪球。连通块大小分布：2 连 76% / 3 连 14% / 4 连 4% / 5+ 连 3%（指数大奖够得着但不廉价）。
