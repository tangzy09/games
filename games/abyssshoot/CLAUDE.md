...
**P1 纯逻辑内核 + P1b 可玩壳 + P2a-1 动画/音效 + P2b-1 存档/最高分/鱼图鉴 + P2b-2 金币/三道具已完成 —— 浏览器里能真玩、能续玩、能收集、能救场了**（仓库根起 http 服 → `http://localhost:8080/games/abyssshoot/`）。
已有：`js/{tiles,core,storage,codex,tools,render,main}.js` + `index.html`（含 `#panel` 图鉴浮层 + `#toasts`）+ `css/game.css` + `locales/{en,zh-CN}.json`（17 条鱼名 + 图鉴/最高分/道具文案）+ `assets/audio/*.wav`（9 个合成音效）+ `assets/fish/*.webp`（17 条鱼图，复用 fishId）；测试：`npm run test:abyss`（单测+蒙特卡洛，含 storage/codex/tools）、`npm run test:abyss:e2e`（Playwright 无头整局，含动画/图鉴/存档续玩/道具流程验证）。
版本化存档（`SAVE_V=2`）落在 `Platform.storage` 键 `abyss_save`：最高分 + 最深鱼 + 金币 + 图鉴 seen 集 + 累计统计 + 当局快照（含 RNG 游标 seed+rolls，可中途关页精确续玩）。
**还没做**：皮肤、成就、每日盘、iOS 壳 —— 见下方「DESIGN 里已定但未实现」与 DESIGN.md 的 P3/P4。
