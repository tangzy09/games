...
// 停滞两级阶梯:>STALL_STEPS:目标改打最近未揭格;
// >STALL_STEPS*2:弃捷径转纯回路(必然扫到未揭格后自动恢复)。
...
function nextMove(s, cyc, mem) {
  ...
  // 此处不做 margin 检查——安全性由 min-fwd(取最小前向跳)+ survivalMove 终检兜底;
  // 仅在开局蛇短、头不在回路方向上时触发。这是全文件唯一不过安全不变式的前向跳格点。
  let move = null;
...
// 安全不变式:候选格前向距离 ≤ 头→尾前向距离 − 余量(身体全部留在前向区间之外);
