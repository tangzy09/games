...
### 激励视频六个位(2026-08-01 加厚版,奖励给得很足)

| 位置 | 奖励 | 类别 | 额度 |
|---|---|---|---|
| 图鉴页 | **+5 张天使** | 收集 | 5 次/天(= 每天最多 +25 张) |
| 每日礼物弹窗 | **+3 张天使** | 收集 | 1 次/天(礼物本身每天一次) |
| **开局礼包**(READY 屏,🎁 与 AI 键并排) | **3 个随机特殊果效果**立刻生效(护盾/光环/磁力…) | **局内增益** | 3 次/天 |
| **任务面板** | **直接完成一个今日任务**(含其奖励) | **任务进度** | 1 次/天 |
| **streak 补签**(每日礼物弹窗,仅断签当天出现) | 把 giftStreak 接回 `prev+1` | **习惯保护** | 恰好漏 1 天时 |
| 死亡后复活 | 复活 + **2 层护盾 + 8s 穿身无敌** | 救场 | 2 次/关 |

- ⛔ **绝不作为奖励发放**:星级 / 单局成就 `r_*` / 纪录——与 AI 免费那条边界同源(**放开进度,锁住「本事的证明」**)。
- **额度是设计不是抠门**:奖励给厚了就必须有 `save.ads{day,gal,boost,quest}` 每日额度,否则一天几十条广告能把 500 张图鉴刷穿、当天毕业,长线直接没了。额度跨天自动清零,UI 上明写「今日还剩 N 次」。
- **拒绝广告 ⇒ 零发放且不扣额度**(冒烟钉死)。发放口统一在 `grantAngels(n)`。
