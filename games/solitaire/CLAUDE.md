...
## ⭐ 激励视频：加厚 + 全部带广告标识（2026-08-03，横幅关掉后它就是主要收入）

**红线一个字没动**（撤销/提示/重开/换局/证明器永远免费），改的只有「给多少」和「怎么说」。

- **加厚 ~50%**：金币 100→**150** · 图鉴 12→**18** · 结算礼包 max(×3,80)+3👼 → **max(×4,120)+5👼** ·
  每日礼物 100+5 → **200+8** · 透视 30s→**60s**（额度 3→4）。救场（3 张拉满）与外观券（任选一款）
  已经到顶，只跟着把附送的透视时长提到 60s。
- ⭐ **每日首看 ×2**（`FIRST_X2`，只对数量型奖励）：每天**一条**翻倍 ⇒ 对长线冲击 = 多给一次的量，
  却把「今天先看一条」的门槛降到最低。**明写在按钮上**（金框 + `×2` 角标），不是暗改。
  ⚠ 倍率必须在**发放那一刻**再算并落位 —— 点击时算好传进去的话，两条广告同时在飞会兑现两次。
- ⭐ **八个位共用 `Render.adRow()`**（原来一半的位既没说是广告、也没说给什么）：
  **① AD 徽章**（`video-ad` 图标 + `sol.adBadge`）**② 具体奖励数量**（「+150 金币」，不是「看广告领币」）
  **③ 今日剩余次数**（用尽时**不消失**，灰掉写「明天再来」—— 按钮突然不见了只会被当成坏了）。
  AdMob 政策也要求激励广告明示「是广告」+「给什么」⇒ **合规与转化同向**。
  ⚠ 两个位放不下整行、自绘但口径一致：**卡死弹窗**（白卡上要绿底，金色看不见）、
  **局内透视钮**（52px ⇒ 左上角 AD 角标 + 底行写奖励秒数）。
  ⚠ `adRow` 里两条抗长语言的处理（德/俄实拍抓出）：奖励文案**逐级降字号**（13→9）而不是截断
  ——奖励被截掉一半等于没说；右侧「今日还剩 N 次」超过行宽 32% 就退化成 `N×`，**主角永远说得完整**。
- **验收**：`node games/solitaire/tools/shot-ads.cjs [lang]` —— 七屏逐个截图 + 断言每个位的可点区都在
  （`node … de` / `ru` 跑长语言）。E2E `npm run test:sol:boost` 新增两条：**首看 ×2 真兑现**
  + **第二条起回到常量表**（防「永久双倍」的退化实现）。
- ⚠ **金币出口目前 7685 币**（61 款收藏品）。顶格玩家（每天看满 13 条 + 赢 6 局）约 **5 天**买光，
  之后金币位对他们就没吸引力了 ⇒ **下次加厚前先扩容出口**（新皮肤，或让金币能换天使）。
