...
- ⚠ **发牌动画期间 `dispatch("STOCK")` 会被吃掉**（连发两次仍写着「5 deals left」）⇒
  先等发牌落定再一次一次发。
- ⛔ **平坦大色块会「闪」**（用户看片发现，实测确认）：牌桌是一整片平滑绿渐变，而 Chrome
  recordVideo 出的 webm 本身是 VP8 有损 —— 静止画面上每帧量化噪声不同，大色块就逐帧微跳亮度
  （同一块 60×60 felt：帧间跳变 >0.15 的有 **118 帧**）。⚠ **不是 x264 的锅**：crf 20/16/14
  实测完全一样，调 crf 治不了。解法 = **只做时间域降噪** `hqdn3d=0:0:6:6`（空间域给 0，一点不糊）
  ⇒ 118 → 10 帧；实拍对照滑牌帧与图鉴缩略图，锐度无差别。`mux.cjs` 已把这条做成**闪烁门禁**。
